...
- Hand each worker the full `task-*.md` brief from `prompts/data-resolution-pipeline/`, `prompts/review-remediation*/`, or `prompts/upsert-all-records/`; do not summarize or substitute a shorter brief
...
- After each wave or review-remediation batch (including `review-remediation-run2`), run `/review` and loop fix → re-review until DoD PASS; defer nothing from review reports; remediation task agents run `/deslop` on owned files before wave merge; thermo-nuclear reviews expect code-judo restructuring, no duplicated orchestration, and sub-1k LOC files
...
- Texas source parquet in `tmp/texas/`: ~199 files, ~614 MB compressed, ~41.5M rows (~34.7M contribs, ~5.1M expend); `contribs`+`expend` ≈90% of disk
- Parquet→Postgres expansion was ~70–90 GB pre-remediation (`raw_data` JSON per txn, one source row→many normalized rows, 120+ indexes, resolve staging, dead tuples); post Wave 1–2 migrations (raw_data dropped) expect single-digit to low tens GB; empty schema baseline ~15 MB; `db_size_report.py` `idx_scan` is lifetime cumulative not row count; reclaim via `scripts/db_reclaim.py` + `cf resolve prune --keep 1`
- Full Texas intake: `cf migrate` → `cf prepare texas --headless --skip-download` → `cf --verbose load texas --preset production` → `scripts/verify_ingest.py`; vectorized load ~9 stages—step 3 FlatTxnsDims scans all RCPT/EXPN (~40M rows) building address/person/entity dims (often hours) before step 4 FlatTxns writes `unified_transactions`; Rich stage progress on TTY (`--no-progress` to disable); smoke use `--preset development`; verbose flag goes on `cf`, not `load`
- `cf load` is append-only (dimension dedup, no txn upsert) until upsert-all-records merges (plan `.cursor/plans/upsert_all_records_e5d0e7fb.plan.md`, brief `prompts/upsert-all-records/`, design `docs/upsert-all-records-plan-2026-06-20.md`; 6-wave Buckets A/B/C/D first-write-wins); clean reload requires `scripts/reset_and_reingest.py`; re-run on same parquet hits unique constraints; never use bare `production_loader.py` (defaults SQLite)—use `cf load`
- `discover_state_files()` anchors relative `tmp/<state>` to repo root not cwd (`scripts/loaders/file_discovery.py`); running `cf load` from `app/` must still discover all parquet files
- Stale `uix_persons_name_state` without `dedup_addr_key` causes mass `UniqueViolation` on `flat_txns_dims`; bootstrap `CREATE UNIQUE INDEX IF NOT EXISTS` won't upgrade—DROP+recreate index with `dedup_addr_key` before full load if index predates Fix 5
- `reset_and_reingest.py` truncates unified tables but not resolution staging—truncate+reload without clearing resolve leaves orphaned `entity_crosswalk`/`canonical_entity`; re-run resolve after unified reload
- Data-resolution pipeline: 11 waves per `.cursor/plans/data_resolution_waves_d3596502.plan.md`; briefs in `prompts/data-resolution-pipeline/`; Phase 0 gate in `app/core/source_models/`, `scripts/loaders/`—Wave 0 must not edit `app/resolve/`; post-`/review` fixes use `.cursor/plans/resolution_pipeline_fixes_ea1848c8.plan.md`
- Resolve pipeline under `app/resolve/`; Phase 1 stages 1→2→3→7; Phase 2 adds 4→5→6; Postgres e2e `uv run --env-file .env python -m app.resolve run|publish --state texas`; stage 1 reads all TX entities (~850k `resolution_input`); `_resolution_schema_models()` must register all staging models or stages 4/6 fail
- Stage 2 blocking: `person_last_phonetic_zip3`, `person_first_initial_last_phonetic`, `org_normalized_zip3`; Postgres `blocking_backend=sql` default; `max_pairs_per_run` default 2M; Stage 4 bulk Splink `predict()`; `canonical_entity.entity_type` uses uppercase Postgres `entitytype` enum
- Alembic `0002_dedup_legacy_transactions` dedups legacy duplicate txns; `tests/core/test_dedup_migration.py` upgrades to `0002` only—`upgrade head` in isolated PG fails when later revisions reference unresolved resolve tables
- GitButler + test gates: main app `uv run pytest tests app/tests --ignore=tests/resolve`; resolve tier `tests/resolve -m "not integration"`; golden CSVs Git LFS; gate task-critic needs `python3 .claude/hooks/gate.py task-critic --session <uuid> --verdict PASS`; never commit `tmp/texas/*.parquet`
