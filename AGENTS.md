...
- `cf load` is append-only (dimension dedup, no txn upsert); clean reload requires `scripts/reset_and_reingest.py`; re-run on same parquet hits unique constraints; never use bare `production_loader.py` (defaults SQLite)—use `cf load`
...
- Pipeline Fix Pack (2026-05-28): role-scoped field prefixes, UnifiedExpenditure, NULL-safe address dedup, 7 dedup indexes via `bootstrap()`; verify with `scripts/verify_ingest.py` (Fix 6 query must use uppercase `RECIPIENT` enum)
