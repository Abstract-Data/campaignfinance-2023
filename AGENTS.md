...
This project is indexed by GitNexus as **campaignfinance** (5463 symbols, 12465 relationships, 286 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- For multi-agent pipeline/remediation work, do not use one monolithic agent to implement wave tasks; dispatch parallel task agents plus one `*z` integrator per wave, run waves sequentially, and have the parent session merge outputs and run `/review` after each wave (orchestrator coordinates only)
...
- After each wave or review-remediation batch (including `review-remediation-round2`), run `/review` and loop fix → re-review until DoD PASS; defer nothing from review reports; remediation task agents run `/deslop` on owned files before wave merge
...
- Post-implementation review remediation: round 1 briefs under `prompts/review-remediation/` with plan `.cursor/plans/review_remediation_waves_1d9dad56.plan.md`; round 2 under `prompts/review-remediation-round2/` (waves 1–5 + `COMPLETION.md`) with plan `.cursor/plans/review_remediation_run_2_cfd210f4.plan.md` and phase branch `remediation-r3`; work uses `remediation/*` GitButler branches and may not be on `gitbutler/workspace`
