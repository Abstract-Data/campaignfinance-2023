...
This worker reproduces — columnar (pure Polars, no ``map_elements`` / ``.apply``)
...
class _OfficerFrame:
    ...
    #                            person_type, _pk_org, _pk_fn, _pk_ln, _full_name
...
def _person_id_map(engine: Any, state_id: int) -> pl.DataFrame:
    """{_pk_org, _pk_fn, _pk_ln} -> person id (lower-cased keys; org-persons keyed on
    lower(org) ALONE via collapse_org_person_key — matches uix_persons_org_state)."""
    tbl = _reflect(engine, "unified_persons")
    stmt = select(tbl.c.id, tbl.c.first_name, tbl.c.last_name, tbl.c.organization).where(
...
def _existing_committee_filer_ids(engine: Any) -> set[str]:
    tbl = _reflect(engine, "unified_committees")
    with engine.connect() as conn:
        return {m["filer_id"] for m in conn.execute(select(tbl.c.filer_id)).mappings().all()}
...
class FilerWorker:
    ...
    def _write_officer_persons(self, parties: pl.DataFrame, ctx: FamilyContext) -> int:
        ...
        new = deduped.join(
            existing.select("_pk_org", "_pk_fn", "_pk_ln"),
    ...
    def _write_committee_persons(
        ...
        for of in officers:
            ...
            f = f.join(person_map, on=["_pk_org", "_pk_fn", "_pk_ln"], how="left", join_nulls=True)
