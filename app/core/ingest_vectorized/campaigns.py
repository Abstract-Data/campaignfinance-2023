...
DEBT / CRED / TRVL / ASSET / PLDG — the ones that produce a UnifiedTransaction):
  ...
  * campaign_name = candidate.full_name (the CANDIDATE-role participant) else
    committee.name. For Texas there is NO ``campaign_name`` source field, and NO
    record type that builds a transaction populates the CANDIDATE role (CAND rows are
    routed to the enrichment path and never call build_campaign), so candidate is
    ALWAYS None here -> campaign_name = committee.name.
...
only ~10 of the ~72 principled distinct keys. This module emits the principled,
...
def _committee_entity_frame(session: Any, state_id: int) -> pl.DataFrame:
    ...
    rows = session.execute(
        select(UnifiedEntity.id, UnifiedEntity.committee_id, UnifiedEntity.entity_type)
...
def finalize_campaigns(session: Any, state_id: int) -> dict[str, int]:
    ...
    campaigns = (
        joined.sort(["_norm", "_committee_id", "_txn_year"], nulls_last=True)
        .unique(
