...
Contains Splink model training, DuckDB candidate-pair staging, exact-pair
prediction, explanation building, and the per-entity-type streaming
orchestration. Extracted from ``score.py`` as part of the score-decomposition
refactor.

Public API:
  - ``load_entity_config`` — dynamically import a per-entity Splink config module.
  - ``score_entity_type_streaming`` — train and score one entity type at scale.
...
from app.resolve.stages.score_bulk import _COPY_SCORED_SQL, bulk_insert_scored
from app.resolve.stages.scored_pair import _SCORED_PAIR_BATCH_SIZE
