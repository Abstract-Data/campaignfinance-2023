...
class OklahomaLobbyistExpenditure(OklahomaSettings, table=True):
    ...
    validate_dates = field_validator("expenditure_date", mode="before")(
        ...
        if "meal_type" in values:
            if values["meal_type"].upper() in ["BREAKFAST", "DINNER", "LUNCH", "OTHER"]:
                ...
                raise PydanticCustomError(
                    ...
                    {"column": "meal_type", "value": values["meal_type"]},
                )
    ...
    def validate_recipient_type(cls, values):
        if "recipient_type" in values:
            if values["recipient_type"].upper() in [
                "LEGISLATOR",
                "NON-LEGISLATOR STATE OFFICER OR EMPLOYEE",
            ]:
                return values
                ...
                raise PydanticCustomError(
                    ...
                    {"column": "recipient_type", "value": values["recipient_type"]},
                )
