...
class CandidateData(TECSettings, table=True):
    ...
    srcCorpContribFlag: Optional[bool] = Field(
        ..., description="Expenditure from corporate funds indicator"
    )
    capitalLivingexpFlag: Optional[bool] = Field(..., description="Austin living expense indicator")
    candidatePersentTypeCd: str = Field(
        ..., description="Type of candidate name data - INDIVIDUAL or ENTITY"
    )
    candidateNameOrganization: Optional[str] = Field(
        ..., description="For ENTITY, the candidate organization name"
    )
    candidateNameLast: Optional[str] = Field(
        ..., description="For INDIVIDUAL, the candidate last name"
    )
    candidateNameSuffixCd: Optional[str] = Field(
        ..., description="For INDIVIDUAL, the candidate name suffix (e.g. JR, MD, II)"
    )
    candidateNameFirst: Optional[str] = Field(
        ..., description="For INDIVIDUAL, the candidate first name"
    )
    candidateNamePrefixCd: Optional[str] = Field(
        ..., description="For INDIVIDUAL, the candidate name prefix (e.g. MR, MRS, MS)"
    )
    candidateNameShort: Optional[str] = Field(
        ..., description="For INDIVIDUAL, the candidate short name (nickname)"
    )
    candidateHoldOfficeCd: Optional[str] = Field(..., description="Candidate office held")
    candidateHoldOfficeDistrict: Optional[str] = Field(
        ..., description="Candidate office held district"
    )
    candidateHoldOfficePlace: Optional[str] = Field(..., description="Candidate office held place")
    candidateHoldOfficeDescr: Optional[str] = Field(
        ..., description="Candidate office held description"
    )
    candidateHoldOfficeCountyCd: Optional[str] = Field(
        ..., description="Candidate office held country code"
    )
    candidateHoldOfficeCountyDescr: Optional[str] = Field(
        ..., description="Candidate office help county description"
    )
    candidateSeekOfficeCd: Optional[str] = Field(..., description="Candidate office sought")
    candidateSeekOfficeDistrict: Optional[str] = Field(
        ..., description="Candidate office sought district"
    )
    candidateSeekOfficePlace: Optional[str] = Field(
        ..., description="Candidate office sought place"
    )
    candidateSeekOfficeDescr: Optional[str] = Field(
        ..., description="Candidate office sought description"
    )
    candidateSeekOfficeCountyCd: Optional[str] = Field(
        ..., description="Candidate office sought county code"
    )
    candidateSeekOfficeCountyDescr: Optional[str] = Field(
        ..., description="Candidate office sought county description"
    )
    ...
    def check_candidate_type(cls, values):
        if values["candidatePersentTypeCd"] not in ("INDIVIDUAL", "ENTITY"):
            raise PydanticCustomError(
                ...
                {"column": "candidatePersentTypeCd", "value": values["candidatePersentTypeCd"]},
            )
        return values
