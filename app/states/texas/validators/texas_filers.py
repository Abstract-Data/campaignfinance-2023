...
class TECFilerLink(TECSettings, table=True):
    ...
    filer_identity_id: int = Field(
        foreign_key="texas.tx_fileridentity.filerIdent", primary_key=True
    )
...
class TECFilerIdentity(TECSettings, table=True):
    ...
    filer_name: list["TECFilerName"] = Relationship(
        back_populates="filer_name", link_model=TECFilerLink
    )
...
class TECFilerName(TECSettings, table=True):
    ...
    filerAddress: TECAddress = Field(default=None, description="Filer address", sa_type=JSON)
    filerMailing: TECAddress = Field(
        default=None, description="Filer mailing address", sa_type=JSON
    )

    filer_name: Optional["TECFilerIdentity"] = Relationship(
        back_populates="filer_name", link_model=TECFilerLink
    )
...
class TECTreasurer(TECSettings, table=True):
    ...
    treasAddress: Optional[TECAddress] = Field(
        default=None, description="Treasurer address", sa_type=JSON
    )
    treasMailing: Optional[TECAddress] = Field(
        default=None, description="Treasurer mailing address", sa_type=JSON
    )

    treasurer: list["TECFiler"] = Relationship(
        back_populates="treasurer", link_model=TECTreasurerLink
    )
    ...
    def check_for_address(self):
        if not self.treasAddress and not self.treasMailing:
            raise PydanticCustomError(
                ...
                {"address": self.treasAddress, "mailing": self.treasMailing},
            )
        return self
    ...
    def create_key(self):
        ...
        if not _address:
            raise PydanticCustomError(
                "missing_address", "Treasurer address is missing", {"address": _address}
            )
...
class TECFiler(TECSettings, table=True):
    ...
    filerName: TECFilerName = Field(default=None, description="Filer name", sa_type=JSON)
    treasurer: list["TECTreasurer"] = Relationship(
        back_populates="treasurer", link_model=TECTreasurerLink
    )
    asstTreasurer: Optional[TECAsstTreasurer] = Field(
        default=None, description="Assistant Treasurer", sa_type=JSON
    )
