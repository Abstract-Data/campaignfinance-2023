...
class TECPersonName(TECSettings):
    ...
    prefix: Optional[str] = Field(..., description="Prefix", max_length=20)
    last: str = Field(..., description="Last name", max_length=50)
    first: str = Field(..., description="First name", max_length=50)
    suffix: Optional[str] = Field(..., description="Suffix", max_length=20)
    field_pfx: Optional[str] = Field(default=None, description="Field prefix")
    ...
    def validate_name(cls, values):
        values["prefix"] = next(
            (value for key, value in values.items() if "NamePrefixCd" in key), None
        )
        ...
        values["suffix"] = next(
            (value for key, value in values.items() if "NameSuffixCd" in key), None
        )
        return values
    ...
    def validate_address(cls, values):
        ...
        gen_address = {
            k: v for k, v in values.items() if (k.startswith(_pfx) and k not in _mail_keys)
        }
